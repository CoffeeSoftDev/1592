...
$query = "SELECT idUP,Name_Proveedor FROM proveedor,proveedor_udn,compras WHERE idProveedor = id_Proveedor AND idUP = id_UP AND id_CG = 2 AND compras.id_UDN = ? GROUP BY Name_Proveedor";
$sql = $crud->_Select($query,$array,"5");
$id_array = array(); $Name_array = array(); $vuelta = 0;
foreach ($sql as $key => $value) {

  $array = array($idE,$value[0],$date);
  $query = "SELECT ROUND(SUM(Gasto),2) FROM compras WHERE id_CG = 2 AND id_UDN = ? AND id_UP = ? AND Fecha_Compras <= ?";
  $sql_prov = $crud->_Select($query,$array,"5");
  foreach($sql_prov as $gasto_proveedor);
  if ( !isset($gasto_proveedor[0]) ) { $gasto_proveedor[0] = 0; }

  $query = "SELECT ROUND(SUM(Pago),2) FROM compras WHERE id_UDN = ? AND id_UP = ? AND Fecha_Compras <= ?";
  $sql_prov = $crud->_Select($query,$array,"5");
...
$query = "SELECT ROUND(SUM(Gasto),2) FROM compras WHERE id_CG = 2 AND id_UDN = ? AND Fecha_Compras <= ?";
$tsql = $crud->_Select($query,$array,"5");
foreach($tsql as $total_gasto);
if ( !isset($total_gasto[0]) ) { $total_gasto[0] = 0; }

$query = "SELECT ROUND(SUM(Pago),2) FROM compras WHERE id_UDN = ? AND Fecha_Compras <= ? AND id_UP IS NOT NULL";
$tsql = $crud->_Select($query,$array,"5");
    ...
    <div class="table table-responsive" >
        ...
        <table class="table table-striped table-hover table-bordered table-condensed">
            ...
            <tbody>
              <?php
                for ($i = 0; $i < $Cont; $i++) {
                  $array = array($idE,$id_array[$i],$date);
                  $query = "SELECT ROUND(SUM(Gasto),2) FROM compras WHERE id_CG = 2 AND id_UDN = ? AND id_UP = ? AND Fecha_Compras <= ?";
                  $sql = $crud->_Select($query,$array,"5");
