...
function lsCheque($obj){
    ...
    # Lista de productos 
    $iptDate = isset($_POST['iptDate']) ? $_POST['iptDate'] : date('Y-m-d');
    $ls = $obj->lsCheques([$iptDate]);
...
function rptGeneral($obj){
    # Declarar variables
    $__row   = [];
    $date      = isset($_POST['iptDate']) ? $_POST['iptDate'] : date('Y-m-d');
   ...
   $__row_propina = [];
    ...
    $lsfolio = $obj  -> lsFolio([ $date ]);
   
    return [
        'table_rpt'     => $table_rpt,
        'table_fp'      => $table_fp,
        'table_propina' => $table_propina,
        'Observaciones' => isset($lsfolio['Observacion']) ? $lsfolio['Observacion'] : ''
